...
internal sealed class DockForm : Form
{
    ...
    private void ToggleLaunchpad()
    {
        ...
        this.launchpadForm.ShowLaunchpad(Screen.PrimaryScreen.WorkingArea, this.Bounds, topMost);
    }
...
internal sealed class LaunchpadForm : Form
{
    ...
    protected override void OnMouseWheel(MouseEventArgs e)
    {
        ...
        this.scrollOffset = Math.Max(0, Math.Min(this.maxScroll, this.scrollOffset - e.Delta / 120 * S(86)));
        this.Invalidate();
    }
    ...
    public void ShowLaunchpad(Rectangle workArea, Rectangle dockBounds, bool topMost)
    {
    ...
    public void PositionLaunchpad(Rectangle workArea, Rectangle dockBounds)
    {
        ...
        if (this.Size != this.targetBounds.Size)
        {
            ...
            this.scrollOffset = Math.Max(0, Math.Min(this.scrollOffset, CalculateMaxScroll()));
        }
    ...
    private void OnAnimationTick(object sender, EventArgs e)
    {
        ...
        this.Opacity = Math.Max(0.0, Math.Min(0.96, 0.96 * eased));
        this.Invalidate();
    ...
    private void DrawLaunchpad(Graphics g)
    {
        ...
        using (StringFormat titleFormat = new StringFormat())
        {
            ...
            RectangleF titleRect = new RectangleF(content.Left, S(12), content.Width, S(34));
            g.DrawString("启动台", titleFont, titleBrush, titleRect, titleFormat);
        }
        ...
        using (StringFormat countFormat = new StringFormat())
        {
            ...
            RectangleF countRect = new RectangleF(content.Left, S(12), content.Width, S(34));
            g.DrawString(this.apps.Count.ToString(CultureInfo.InvariantCulture) + " apps", countFont, countBrush, countRect, countFormat);
        }
    ...
    private void DrawApps(Graphics g, Rectangle content)
    {
        ...
        int firstRow = Math.Max(0, this.scrollOffset / Math.Max(1, metrics.RowStride) - 1);
        int lastRow = Math.Min(metrics.Rows - 1, (this.scrollOffset + content.Height) / Math.Max(1, metrics.RowStride) + 1);
    ...
    private void DrawAppTile(Graphics g, LaunchpadApp app, RectangleF tile, bool hovered, Font labelFont, Brush labelBrush, StringFormat labelFormat)
    {
        ...
        if (app.Bitmap != null)
        {
            g.DrawImage(app.Bitmap, iconRect);
        }
        else
        {
    ...
    private void DrawScrollBar(Graphics g, Rectangle content)
    {
        ...
        float thumbTop = content.Top + (trackHeight - thumbHeight) * this.scrollOffset / Math.Max(1, this.maxScroll);
    ...
    private int FindAppAtPoint(Point point)
    {
        ...
        GridMetrics metrics = CalculateGridMetrics(content);
        int localY = point.Y - content.Top + this.scrollOffset;
    ...
    private static List<LaunchpadApp> LoadStartMenuApps()
    {
        List<LaunchpadApp> apps = new List<LaunchpadApp>();
        ...
        {
            AddStartMenuAppsFromRoot(apps, seen, roots[i]);
        }

        apps.Sort(delegate(LaunchpadApp left, LaunchpadApp right)
        {
    ...
    private static void AddStartMenuAppsFromRoot(List<LaunchpadApp> apps, HashSet<string> seen, string root)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                apps.Add(new LaunchpadApp
                {
                    Name = name,
                    Command = files[i],
                    Bitmap = LoadShortcutBitmap(files[i], name)
                });
            }
        }
        catch (Exception ex)
    ...
    private static Bitmap LoadShortcutBitmap(string path, string label)
    {
        try
        {
